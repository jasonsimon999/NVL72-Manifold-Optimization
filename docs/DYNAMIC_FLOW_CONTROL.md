...
## September 22 correction to sizing and comparison

The earlier description of a square-root *area* correction is superseded. Adaptive sizing now uses the same permanent orifice loss law as the network: `K_new = K_current (mass_actual / mass_target)^2`. Invert K to bore, convert bore to actuator area position, enforce travel limits, and re-solve the coupled network. This is a local pressure approximation and feedback controller, not a proven global optimizer.

The page shows fixed, adaptive trial, stationary active valves, and selected design separately. Stationary valves retain body loss, holding power, electronics and capital cost. Selecting a fixed design is an offline design recommendation, not active valves magically losing their overhead. Acceptance protects peak solid/outlet temperature, auxiliary power, peak pressure and RMS/worst flow errors. Fault runs show actual faults without replacing them with a passive result. No individual configuration is certified against unknown manufacturer limits.

Individual service schedules overlay any workload. Each selected tray has a disconnect time, optional reconnect time and positive reconnection ramp. The ramp scales connection area and workload together; it is an adjustable operating assumption. Heat pulses now use their entire requested duration within the run; zero burst duty produces no pulses. Event resolution is the simulation timestep. The sidebar exposes all advanced assumptions; bore/flow maps show every tray and the raw adaptive trial even when fixed hardware is selected. A no-plate branch uses tube ID for the bore display.
