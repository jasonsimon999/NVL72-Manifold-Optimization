...
### 2026-09-22 — beginner-facing progressive disclosure

User requested simpler UI across both pages. Fixed sidebar leaves rack flow/header diameter visible; orifice and tray geometry remain in a closed expander. Dynamic sidebar leaves reference/workload/duration visible, with controller, pump, prices and servicing folded away. Run remains before advanced form controls. Secondary candidate comparisons, time traces, all-tray tables, cost scenarios and reliability details are collapsed; rack map, headline results and bore history remain prominent. All controls still execute when collapsed, preserving defaults, saved values, and model behavior. Tab names and captions use shorter labels. No physics or default numerical parameters changed for this UI request.

### 2026-09-22 — follow-up active calculation audit

See `docs/DYNAMIC_AUDIT.md` and reproduce `studies/audit_dynamic.py`. Runtime checks now enforce identical fixed/active time, electrical/liquid heat, service and thermal-target arrays, plus constant passive bores. Shared thermal demand includes the tray solid setpoint and outlet limit using Ts=Tin+P/(m cp)+RP. Unreachable setpoints are flagged rather than dividing by zero. Active sizing applies bounded positive temperature trim, the same inverse orifice law, actuator limits and network re-solve. Combined service ramps apply heat scaling only once; initial connections are respected; per-tray thermal summaries exclude disconnected trays. Stored heat export is independently computed from temperature increments.

The five audited cases do NOT establish universal active thermal improvement. Positive peak reduction only appeared in the combined service/spike case (+0.079 K), whose mean was 1.438 K warmer. Fixed was selected in all five. Keep raw adaptive results visible, and never report passive selection as a proven active-control benefit. The report contains all numerical settings, results and limitations. New UI diagrams show both actual forcing arrays, both bore histories on identical scales, and selected-tray heat/bore/temperature overlays. 77 tests passed after these changes.

Timestep refinement of the combined service/spike case at 2/1/0.5 s gives peak reductions 0.079/0.118/0.111 K, but mean remains about 1.43 K warmer. At 0.5 s active auxiliary power is 329.73 W versus fixed 318.89 W: the apparent 2 s power saving is not robust. See timestep_refinement.csv; do not claim overall improvement.
